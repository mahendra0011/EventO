...
function createTextTexture(gl, text, font = 'bold 30px monospace', color = 'black') {
  const label = text.length > 34 ? `${text.slice(0, 31).trim()}...` : text;
  ...
  context.strokeStyle = 'rgba(18, 15, 13, 0.65)';
...
class Title {
  ...
  createMesh() {
    ...
    this.mesh = new Mesh(this.gl, { geometry, program, renderOrder: 10 });
  ...
  onResize() {
    ...
    const textHeight = 0.085;
    const textWidth = Math.min(textHeight * this.aspect, 0.86);
...
class CircularGalleryApp {
  ...
  update() {
    ...
    this.renderer.render({ scene: this.scene, camera: this.camera, sort: false });
